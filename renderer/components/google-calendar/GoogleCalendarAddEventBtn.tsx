...
export default function GoogleCalendarAddEventBtn({ addEvent }) {
  ...
  const generateMenuEvents = () => {
            ...
            <div className="">
              <Menu.Item key={id}>
                {({ active }) => (
                    ...
                    >
                      {summary}
                      <span className={`${ active ? 'text-white' : 'text-gray-500'} block text-xs`}>
    ...
    return <div className="text-gray-500 text-xs p-2 text-center">Connect to Google Calendar to get Events</div>
  }
  ...
  const getTimeAsInt = (date: string = ''): number => {
    ...
    return parseInt(curDate.getHours() + '' + ('0' + curDate.getMinutes()).substr(-2));
  }

  const getTodayEvents = (): any[] => {
    const { google } = calendars;
    if (google && google.length > 0) {
      return google.filter((event) => {
        const { start, end } = event;
        const from: { date: string, time: string } = setDateTimeObj(start.dateTime);
      ...
      <div className="">
        <Menu as="div" className="relative inline-block text-left">
          <div className="inline-flex w-full justify-center rounded-md bg-black bg-opacity-20 hover:bg-opacity-30">
            <button type="button" className="px-4 py-2 text-sm font-medium text-white border-r-2" onClick={addActiveEvent}>Add Event</button>
