import {
  updateJiraAccessToken,
  removeJiraStoredUser,
  updateJiraStoredUser
} from '../jira';
...
describe('GIVEN jira/updateJiraAccessToken', () => {
  ...
  it('calls ipcRenderer.invoke with the correct arguments', async () => {
    ...
    expect(global.ipcRenderer.invoke).toHaveBeenCalledWith(
...
describe('GIVEN jira/removeJiraStoredUser', () => {
  ...
  it('removes a user from localStorage when there are multiple users', () => {
    ...
    const updatedUsers = JSON.parse(
      localStorage.getItem(LOCAL_STORAGE_VARIABLES.JIRA_USERS)
    );
  ...
  it('removes the entire localStorage key when removing the last user', () => {
    ...
    const updatedUsers = localStorage.getItem(
      LOCAL_STORAGE_VARIABLES.JIRA_USERS
    );
